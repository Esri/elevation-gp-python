...
class Viewshed(object):
    def __init__(self):
        ...
        #Data source layers
        #--------------------------------------------        
        self.dictMosaicLayers = {'30':r'C:\elevation-gp-python-master\ElevationPro\Data\Elevation.gdb\dem30m', 
                                '60':r'C:\elevation-gp-python-master\ElevationPro\Data\Elevation.gdb\dem60m', 
                                '90':r'C:\elevation-gp-python-master\ElevationPro\Data\Elevation.gdb\dem90m'}
        self.resolutionCoverage = r'C:\elevation-gp-python-master\ElevationPro\Data\Boundary.gdb\databoundary'
        #--------------------------------------------
    ...
    def ContainmentCheck(self, in_point_or_buffer):
        arcpy.management.MakeFeatureLayer(self.resolutionCoverage, 'databoundary_containment', 'polytype = 0 Or polytype = 1')
    ...
    def CreditCheck(self, in_points, dem_res):
        arcpy.management.MakeFeatureLayer(self.resolutionCoverage, 'databoundary_credit', 'polytype = 2')
    ...
    def formatInputDEMSource(self, inSource):
        tempDEMList = list(self.dictDEMSources.keys())
        tempDEMList.extend(["", "FINEST"])
        retVal = inSource
    ...
    def executeVisibility(self, mosaic_layer, in_points, obs_count, in_buffer, credit_list, maximum_distance,
        ...
        try:
            ...
            scratchWS = "in_memory"
            outvsd0 = os.path.join(arcpy.env.scratchFolder, "viewrastmp0.tif")
            outvsd = os.path.join(arcpy.env.scratchFolder, "viewrastmp")
            ...
            # Add metadata info
            dem_source = [k for k, v in self.dictDEMSources.items() if v == str(dem_resolution)]
    ...
    def getParameterInfo(self):
        ...
        list_dem = ["FINEST"]
        dem_keys = list(self.dictDEMSources.keys())
        dem_keys.sort()
